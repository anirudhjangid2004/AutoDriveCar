...
/*
    Function to convert uint32_t to uint8_t array
    @param value: Value to convert
    @param array: Array to store the converted value
    @param size: Size of the array
*/
void uint32_to_uint8(uint32_t *value, uint8_t *array, size_t size) {
    for(size_t j = 0; j < size; j++){
        for (size_t i = 0; i < 4; i++) {
            array[4*j + i] = (value[j] >> (i * 8)) & 0xFF;
        }
    }
}

/*
    Function to convert uint16_t to uint8_t array
    @param value: Value to convert
    @param array: Array to store the converted value
*/
void uint16_to_uint8(uint16_t *value, uint8_t *array, size_t size, size_t offset) {
    for(size_t j = 0; j < size; j++){
        for (size_t i = 0; i < 2; i++) {
            array[2*j + i + offset] = (value[j] >> (i * 8)) & 0xFF;
        }
    }
}
...
/*
    Main function
    Initializes the USB stack, motor pins, interrupt pins, and IMU
    Waits for USB connection and then enters main loop
*/
int main() {
    ...
    uint8_t i = 0;
    // This array will contains the encoder and imu values
    uint8_t odomData[32] = {0};
    ...
    while (!kill_me) {
        ...
        readUSBBuffer();
        uint32_to_uint8(REVOLUTIONS, odomData, 4);
        uint16_to_uint8(accel_final, odomData, 3, 16);
        uint16_to_uint8(gyro_final, odomData, 3, 22);
        ...
        i++;
        // push_message("\n");
        sendUSBMessage(TUSB_MSG_ID_ODO_DATA, odomData, 32);
        sleep_ms(5);
    }
